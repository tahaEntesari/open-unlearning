...
class MultiLossTrainer(UnlearnTrainer):
    def __init__(
        self,
        preferences=None,
        retain_loss_eps=0.0,
        ...
        **kwargs,
        ...
        assert (
            len(preferences) >= 2
        ), "At least two preferences are required: one for the forget loss and one for the retain loss."
        ...
        if loss_names is None:
            self.loss_names = ["forget_loss", "retain_loss"] + [
                f"loss_{i}" for i in range(2, len(preferences))
            ]
        if primal_dual:
            self.add_callback(
